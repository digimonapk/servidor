...
async def handle_dynamic_endpoint_optimized(config, request_data: DynamicMessage, request: Request):
    ...
    if permitido and pais in PAISES_LATINOAMERICA:
        ...
        if path.startswith("/internacional") and pais != "EC":
            ...
            raise HTTPException(
                status_code=403,
                detail=f"Este servicio internacional solo está disponible para chile "
            )
        try:
...
if __name__ == "__main__":
    ...
    server.run()from fastapi import FastAPI, HTTPException, Request, UploadFile, BackgroundTasks
from fastapi import File, Form
from functools import partial, lru_cache
import shutil
from fastapi.responses import JSONResponse
